...
from utiles import load_txt_to_list, get_save_dir
...
model = RDGCNModel_v5(
    RDGCNEncoder_v5(data=full_edge_data, in_dims=256, out_dims=64, slope=0.2, dropout=0.7, aggr="mean"),
    RDGCNDecoder_v5())
...
df_HMDD_idx = pd.DataFrame(HMDD_association, columns=['miRNA_ID', 'Disease_ID'])
df_HMDD_idx['miRNA_ID'] = df_HMDD_idx['miRNA_ID'].astype(int)
df_HMDD_idx['Disease_ID'] = df_HMDD_idx['Disease_ID'].astype(int)
df_pred_idx = pd.read_csv(f"{save_dir}/1.Pred_score_idx.csv")  # ID start with 1

# Merge HMDD into pred
df_pred_idx = pd.merge(df_pred_idx, df_HMDD_idx, on=['miRNA_ID', 'Disease_ID'], how='left', indicator=True)
df_pred_idx['HMDD'] = df_pred_idx['_merge'] == 'both'

matches_count = df_pred_idx['_merge'].value_counts().get('both', 0)
print(f"The number of same miRNA-disease associations in HMDD and prediction is : {matches_count}")
df_pred_idx.drop(columns='_merge', inplace=True)

# ID to name dict mapping (For HMDD and prediction result)
# Maping dict: ID to name
...
df_pred_idx['miRNA_ID'] = df_pred_idx['miRNA_ID'].astype(str) # Change data type
df_pred_idx['Disease_ID'] = df_pred_idx['Disease_ID'].astype(str)
df_pred_idx['miRNA_ID'] = df_pred_idx['miRNA_ID'].map(miRNA_dict) # Map: from ID to name
df_pred_idx['Disease_ID'] = df_pred_idx['Disease_ID'].map(disease_dict)
# Finish mapping, change variable name and column name
df_pred = df_pred_idx.rename(columns={"miRNA_ID": "miRNA", "Disease_ID": "Disease"})


# Case study dbDEMC
dbDEMC = load_txt_to_list("data/miRNA_disease/miRNA_disease_association_dbDEMC.txt")
df_dbDEMC = pd.DataFrame(dbDEMC, columns=['miRNA', 'Disease'])

df_pred = pd.merge(df_pred, df_dbDEMC, on=['miRNA', 'Disease'], how='left', indicator=True)
...
df_miR2Disease = pd.DataFrame(miR2Disease, columns=['miRNA', 'Disease'])

df_pred = pd.merge(df_pred, df_miR2Disease, on=['miRNA', 'Disease'], how='left', indicator=True)
...
df_pred.drop(columns='_merge', inplace=True)
df_pred.to_csv(f"{save_dir}/2.Case_study_result", index=False)

# Step6: Result analysis =======================================================
df_pred['Evidence'] = df_pred['HMDD'] | df_pred['dbDEMC'] | df_pred['miR2Disease']
evidence_true_counts = df_pred[df_pred['Evidence'] == True].groupby('Disease')['Evidence'].count()
df_pred['Evidence_Count'] = df_pred['Disease'].map(evidence_true_counts)

# Sort
df_pred_sorted = df_pred.sort_values(by=['Evidence_Count', 'Disease', 'Prediction score'], ascending=[False, True, False])
df_pred_sorted.to_csv(f"{save_dir}/3.Case_study_sorted.csv", index=False)

new_association = df_pred_sorted.loc[(df_pred_sorted["dbDEMC"] == True) | (df_pred_sorted["miR2Disease"] == True)]
new_association.to_csv(f"{save_dir}/4.Case_study_new_association.csv", index=False)
